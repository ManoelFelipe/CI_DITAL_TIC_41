...
// =============================================================================
// ULA_DATAPATH — Implementação DATAFLOW
// Lógica combinacional expressa com 'assign'; registrador mantido como submódulo.
// =============================================================================
module ula_datapath (
    ...
    wire [3:0] ula_res_df;
    wire       ula_carry_df;
    wire [3:0] mux_out_df = (sel21) ? ula_res_df : dados;

    // Registrador
    wire [3:0] reg_q_df;
    reg4 UREG (.clk(clk), .reset(reset), .d(mux_out_df), .q(reg_q_df));

    // DEMUX (assign)
    wire [3:0] A_df = (sel12 == 1'b0) ? reg_q_df : 4'b0000;
    wire [3:0] B_df = (sel12 == 1'b1) ? reg_q_df : 4'b0000;

    // ULA
    ula4 UALU (.A(A_df), .B(B_df), .operacao(operacao),
               .resultado(ula_res_df), .carry_out(ula_carry_df));

    assign resultado = ula_res_df;
    assign carry_out = ula_carry_df;
endmodule
