...
Crie o módulo **`ff_jk`** em **três abordagens** — *Behavioral*, *Dataflow* e *Structural* — seguindo **exatamente**:
...
```verilog
// ============================================================================
// Arquivo  : ff_jk  (implementação [ABORDAGEM])
...
- Para o tb_ff_jk do Testbench também precisa ter cabeçalho.
...
if {$IMPLEMENTATION eq "behavioral"} {
    vlog -work work ../rtl/behavioral/ff_jk
} elseif {$IMPLEMENTATION eq "dataflow"} {
    vlog -work work ../rtl/dataflow/ff_jk
} elseif {$IMPLEMENTATION eq "structural"} {
    vlog -work work ../rtl/structural/ff_jk
} else {
    echo "IMPLEMENTATION invalido: $IMPLEMENTATION"
    return
}

vlog -work work ../tb/tb_ff_jk
```
