...
```
Projeto_count_sync_pwm/
├── README.md
├── Quartus/
│   └── rtl/
│       ├── behavioral/count_sync_pwm.v
│       ├── dataflow/count_sync_pwm.v
│       └── structural/count_sync_pwm.v
└── Questa/
    ├── rtl/
    │   ├── behavioral/count_sync_pwm.v
    │   ├── dataflow/count_sync_pwm.v
    │   └── structural/count_sync_pwm.v
    ├── tb/tb_count_sync_pwm.v
...
```verilog
// ============================================================================
// Arquivo  : count_sync_pwm  (implementação [ABORDAGEM])
...
if {$IMPLEMENTATION eq "behavioral"} {
    vlog -work work ../rtl/behavioral/count_sync_pwm
} elseif {$IMPLEMENTATION eq "dataflow"} {
    vlog -work work ../rtl/dataflow/count_sync_pwm
} elseif {$IMPLEMENTATION eq "structural"} {
    vlog -work work ../rtl/structural/count_sync_pwm
} else {
    echo "IMPLEMENTATION invalido: $IMPLEMENTATION"
    return
}

vlog -work work ../tb/tb_count_sync_pwm
```
...
do clean.do
do compile.do
vsim -voptargs=+acc work.tb_count_sync_pwm
add wave -r /*
run -all
```
