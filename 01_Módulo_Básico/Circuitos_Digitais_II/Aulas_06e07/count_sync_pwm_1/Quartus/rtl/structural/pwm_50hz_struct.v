...
(
    ...
    assign pwm_out =
        (duty_sel == 2'b11) ? 1'b1 :  // 100% em nível alto
                              pwm_raw; // Casos 25%, 50% e 75% usam comparador
...
(
    input  wire       clk,      // Clock de 100 MHz
    input  wire       reset_n,  // Reset assíncrono ativo em nível baixo
    output reg [20:0] count_q   // Saída do valor atual do contador
);

    // Bloco sequencial do contador
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            count_q <= 21'd0;                // Zera contador no reset
        end else begin
            if (count_q >= (COUNTER_MAX - 1)) begin
                count_q <= 21'd0;            // Reinicia quando atinge limite
            end else begin
                count_q <= count_q + 21'd1;  // Incrementa contador
            end
        end
    end
...
(
    input  wire [1:0]  duty_sel,   // Seleção de duty cycle
    output reg  [20:0] threshold   // Limiar correspondente
);

    // Cálculo local dos limiares de 25%, 50% e 75%
    ...
    always @* begin
        case (duty_sel)
            2'b00: threshold = DUTY_25;   // 25%
            2'b01: threshold = DUTY_50;   // 50%
            2'b10: threshold = DUTY_75;   // 75%
            default: threshold = DUTY_100; // 100% (caso 11)
        endcase
    end
...
(
    input  wire [20:0] counter,    // Valor atual do contador
    input  wire [20:0] threshold,  // Limiar de duty cycle
