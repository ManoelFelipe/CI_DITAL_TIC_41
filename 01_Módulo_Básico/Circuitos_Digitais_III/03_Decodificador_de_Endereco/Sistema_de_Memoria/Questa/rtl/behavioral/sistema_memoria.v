...
// Descrição: Sistema de memória endereçável de 128 bytes (7 bits de endereço) com topologia 4x4 de blocos 8x8. 
// As duas linhas superiores são ROM (64 bytes) e as duas inferiores são SRAM (64 bytes). 
// Escrita síncrona na SRAM com WE e CLK; leitura registrada em DOUT para comportamento determinístico em simulação e síntese. 
// A ROM é implementada via função combinacional derivada do endereço (conteúdo fixo), evitando arquivos de inicialização. 
// Riscos: multiplexação grande pode impactar timing; uso de registrador de saída reduz glitches e facilita equivalência entre abordagens.
